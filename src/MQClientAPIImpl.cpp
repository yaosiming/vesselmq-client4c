										...
										PullCallback* pPullCallback)
{
	...
	RemotingCommand* request = RemotingCommand::createRequestCommand(PULL_MESSAGE, pRequestHeader);
	request->Encode();
																...
																int timeoutMillis)
{
	...
	RemotingCommand* request =
		RemotingCommand::createRequestCommand(GET_CONSUMER_LIST_BY_GROUP, requestHeader);
	request->Encode();
	RemotingCommand* response = m_pRemotingClient->invokeSync(addr, request, timeoutMillis);
		...
		case SUCCESS_VALUE:
			{
				...
				if (response->GetBody() != NULL)
				{
					...
					std::cout<<"consumerIdlist:"<<ret.size()<<std::endl;
					delete response;
					delete request;
					delete body;
					return ret;
